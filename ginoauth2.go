...
var IntrospectionURL, ClientId, ClientSecret string
...
func requestAuthInfo(o Options, t *oauth2.Token) ([]byte, error) {
	var infoURL string = IntrospectionURL

	// if o.AccessTokenInHeader {
	// 	infoURL = IntrospectionURL
	// } else {
	// 	var uv = make(url.Values)
	// 	uv.Set("access_token", t.AccessToken)
	// 	infoURL = IntrospectionURL + "?" + uv.Encode()
	// }
	formData := url.Values{}
	formData.Add("token", t.AccessToken)
	formData.Add("token_type_hint", "access_token")
	body := bytes.NewBufferString(formData.Encode())
	client := &http.Client{Transport: &Transport}
	req, err := http.NewRequest("POST", infoURL, body)
	if err != nil {
		return nil, err
	}
	...
	basicKey := base64.StdEncoding.EncodeToString([]byte(ClientId + ":" + ClientSecret))
	req.Header.Set("Authorization", fmt.Sprintf("Basic %s", basicKey))
...
func AuthChainOptions(o Options, accessCheckFunctions ...AccessCheckFunction) gin.HandlerFunc {
	// init
	IntrospectionURL = o.Endpoint.AuthURL
	// middleware
	return func(ctx *gin.Context) {
		...
		go func() {
			...
			if !tokenContainer.active {
				ctx.AbortWithError(http.StatusUnauthorized, errors.New("token not active"))
				varianceControl <- false
			}
